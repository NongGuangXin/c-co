...
- **事件驱动**：基于 Linux epoll 或 io_uring 实现网络I/O多路复用
- **错误处理**：使用 `std::expected` 进行错误处理
- **IO和事件分离**：事件触发和IO操作线程分离
...
- **操作系统**：Linux（需要 epoll 和 io_uring 支持）
...
4. **I/O 挂起**：当遇到I/O操作时，协程挂起并注册到事件反应器
5. **事件唤醒**：当I/O就绪时，事件反应器触发回调通知执行器恢复协程
...
2. **Linux 系统**：使用了 Linux 的 epoll 和 io_uring
3. **多线程**：当前实现使用多事件线程和多处理线程
